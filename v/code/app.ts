...
export abstract class app extends outlook.page {
    ...
    constructor(
        ...
        this.panels
            ...
            .set(
                'message',
                new theme.theme(
                    { ename: 'msg', dbname: this.config.user_subject.dbname },
                    '#message',
                    this
                )
            )
            .set(
                'event',
                new theme.theme(
                    { ename: 'event', dbname: this.config.user_subject.dbname },
                    '#event',
                    this
                )
            );
    ...
    async get_user(): Promise<user | undefined> {
        ...
        if (user_str === null) User = await this.login();
            ...
            if (name !== undefined && pk !== undefined) User = new user(name, pk);
        }
        //
        //return teh user
        return User;
    ...
    private get_role_id(ename: library.ename, dbase: schema.database): boolean {
        ...
        const f_cnames = cnames.filter((cname) => {
            ...
            const test = col instanceof schema.foreign && col.ref === this.config.user_subject;
            //
            return test;
        });
        //
    ...
    async login(): Promise<user | undefined> {
        ...
        if (role_ids.length === 0 || business === undefined) {
            ...
            const result: { role_ids: Array<string>; business: business } | undefined =
    ...
    private async activate_products(): Promise<void> {
        ...
        const subscribed = await server.exec('app', [this.id], 'available_products', [
            this.user!.name!,
        ]);
        //
        //Add the subscribed
        ...
        return [
            {
                ...
                solutions: [
                    ...
                    },
                    {
                        title: 'View Template',
                        id: 'view_template',
                        listener: ['event', async () => await new viewer.viewer(this).administer()],
                    },
                ],
            },
        ];
    ...
    async register(): Promise<Array<string> | undefined> {
        ...
        if (inputs.length === 0)
            throw new schema.mutall_error(
                'No user roles are found in the current database. Check your data model'
            );
        //
        ...
        const html: string = await server.exec('questionnaire', [this.dbname], 'load_common', [
            login_db_data,
        ]);
        //
    ...
    private get_subscription_data(): Array<quest.label> {
        //
        //Get the dbname. (why not ths.dbname? that would be the dbname for
    ...
    async change_subject(selector: HTMLSelectElement): Promise<void> {
        ...
        const subject: schema.subject = { ename, dbname };
        //
    ...
    async relink_user(): Promise<void> {
        ...
        const ok: boolean = await server.exec('tracker', [], 'relink_user', [links]);
        //
    ...
    public get_business_entity(): schema.entity {
        ...
        if (count === 0) throw new schema.mutall_error('Business table missing; incomplete model');
        //
    ...
    async load_data(): Promise<void> {
        ...
        //array of layouts.
        const layouts = <Array<quest.layout>>JSON.parse(text);
        //
        //Load the data specified by the Iquestionnaire (using the common
        //method)
        const html = await server.exec('questionnaire', [this.dbname], 'load_common', [layouts]);
        //
    ...
    async edit_table() {
        ...
        const options: Array<outlook.option<schema.entity>> = entities.map((entity) => ({
            name: entity.name,
            value: entity,
        }));
        //
        ...
        const subject: schema.subject = { ename: selected.name, dbname: this.dbname };
        //
...
//The way of identifying a business.
...
export class user {
    ...
    //password
    constructor(public name: string, public pk: number) {}
    ...
    //Returns the roles of a usee in the given application, and the business
    //under which he is logged in. A user is not registed if either the roles
    //are empty or the business is undefiend.
    async get_roles_and_business(
        App: app
    ): Promise<{ role_ids: Array<string>; business?: business }> {
        //
        ...
        const ids = <Array<{ id: string }>>(
            await server.exec('database', [App.config.user_subject.dbname], 'get_sql_data', [sql])
        );
        //
    ...
    async get_current_business(): Promise<business | undefined> {
        ...
        const businesses: Array<business> = await server.exec(
            'database',
            [schema.entity.user.dbname],
            'get_sql_data',
            [sql]
        );
        //
