...
export abstract class baby<o> extends quiz<o> {
    ...
    async fill_selector(ename: string, dbname: string, selectorid: string) {
        ...
        const options: Array<schema.fuel> = await exec('selector', [ename, dbname], 'execute', []);
        //
...
> extends popup<r> {
    ...
    async show_panels() {
        ...
        if (panel === null) throw new schema.mutall_error('No hook element found for the choices');
        //
