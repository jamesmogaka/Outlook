...
export class view {
    ...
    */
    get_radio_value(env) {
        ...
        if (radios.length < 2)
            throw new mutall_error(`At least 2 radio buttons are expected. ${radios.length} was found`);
        //
    ...
    get_text_value(env) {
        ...
        if (element === null)
            throw new mutall_error('Envelop element does not have an input child element');
        //
    ...
    report_error(key, credentials) {
        //
        //Use the key to get the error message
        const error = credentials[key];
        //
