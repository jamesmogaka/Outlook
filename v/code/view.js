...
export class view {
    ...
    */
    get_value(df_name) {
        ...
        const io_type = this.get_io_type(env);
        //
    ...
    */
    get_envelop(df_name) {
        ...
        const env = this.document.querySelector(`*[data-field = "${df_name}"]`);
        //
        ...
        const target = Array.from(this.document.querySelectorAll('*[data-field]'));
        //
        ...
        const envelop = target.filter((target) => {
            //
            Array.from(target.children).filter((child) => child.getAttribute('name') ||
                child.getAttribute('id') === df_name);
        });
        //
        //Check to see if there was any element at all
        if (envelop.length === 0)
            throw new mutall_error(`No envelop or input element identified by ${df_name} was found`);
        //
        //Ensure that we retrived only one element
        if (envelop.length > 1)
            throw new mutall_error(`We expected only one element identified by the ${df_name} but got ${envelop.length}`);
        //
    ...
    get_io_type(env) {
        ...
        const text = this.extract_io(env) ?? 'text';
        //
    ...
    extract_io(env) {
        ...
        const attributes = Array.from(env.attributes);
        //
        //Search for the data-iotype attribute amongest the envelop attributes
        const io_type = attributes.find((attribute) => attribute.name === 'data-iotype');
        //
        //If the data-iotype exists extract the value and return it
        if (io_type)
            return io_type.value;
        //
        //Otherwise get all the children of the envelop
        const children = Array.from(env.children);
        //
        //From the children extract the child with a type attribute
        const input = children.filter((child) => Array.from(child.attributes).find((attribute) => attribute.name === 'type'));
        //
        ...
        if (input.length > 1)
            throw new mutall_error(`Your envlop has${input.length} elements while we expected it to have one!`);
        //
    ...
    */
    get_radio_value(env) {
        ...
        const checkeds = env.querySelectorAll('input[type=radio]:checked');
        //
    ...
    */
    ...
    report_error(id, msg) {
        ...
        const envelop = this.document.getElementById(id);
        //
        ...
        if (envelop === null) {
            ...
            const general = this.get_element('report');
            //
            ...
            const local = envelop.querySelector('.error');
            //
            ...
            if (!local)
                throw new mutall_error(`This enveloping element '${id}' has no error reporting element`, msg);
            //
